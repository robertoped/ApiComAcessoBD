...
namespace ApiComAcessoBD.Controllers
{
    ...
    public class PessoaJuridicaController : ControllerBase
    {
        ...
        public PessoaJuridicaController(DataBaseContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        {
            IEnumerable<Pessoa> pessoas = _context.Pessoa.Where(x => x.IdTipoPessoa == 2)
        ...
        {
            Pessoa? pessoa = _context.Pessoa.Include(x => x.PessoaJuridica)
                                            .Include(x => x.PessoaTelefones)
                                            .Include(x => x.PessoaEnderecos)
                                            .FirstOrDefault(x => x.Id == id && x.IdTipoPessoa == 2);
        ...
        {
            List<Pessoa>? pessoa = _context.Pessoa.Where(x => x.PessoaJuridica.RazaoSocial.Contains(nome) || x.PessoaJuridica.NomeFantasia.Contains(nome) && x.IdTipoPessoa == 2)
                                                  .Include(x => x.PessoaJuridica)
                                                  .Include(x => x.PessoaTelefones)
                                                  .Include(x => x.PessoaEnderecos)
                                                  .ToList();
